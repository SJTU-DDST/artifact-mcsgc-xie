...
#define SLICES_ZONE (16 * (1 << 16))    // one zone is 16GB, 16G =  (16 * 64K) * 16KB, 
                                        // consisting of 1G slices (i.e. nand pages)
#define SIZE_ZONE_NODE (1 << 10)        // one zone node consists of 1K data nodes (i.e. map node in the paper)
                                        // data node manages 16MB space, i.e. 1K slices
                                        // however, mapping is done in 4KB granularity, thus 4K entries in one data node
...
typedef struct node_model {
    unsigned int invSlope;  // (inv for interval?) lba size of child nodes
    unsigned int bias;      // start lba
} NodeModel;

static inline int calcPosition(NodeModel model, unsigned int logicalSliceAddr) {
...
extern struct unit_allocator tAllocator;
extern struct unit_allocator mapSegmentAllocator;
extern struct unit_allocator rootNodeAllocator;
extern struct unit_allocator rootNodeChildrenAllocator;

static inline void initMappingTree(FunctionalMappingTree *fmTree) {
    fmTree->rootNode = (RootNode *) uallocate(&rootNodeAllocator, 1);
}

static inline void initZoneNode(ZoneNode *node, unsigned int firstItemAddr) {
    node->size = SIZE_ZONE_NODE;
    node->model.invSlope = MAPSEG_MAP_SEGMENT_SIZE;     // 4K LBAs in one map segment
...
static inline void initRootNode(RootNode *node, unsigned int firstItemAddr) {
    node->size = 0;
    node->model.invSlope = SIZE_ZONE_NODE * MAPSEG_MAP_SEGMENT_SIZE;    // 4M LBAs in one data node
    node->model.bias = firstItemAddr;
...
static inline void tryInvalidateZoneNodes(RootNode *node) {
...
static inline void copyChildZoneNodes(ZoneNode **newZoneNodes,
...
static inline void doExpandRootNode(RootNode *node, int count) {
    ...
    node->childZoneNodes = (ZoneNode **) uallocate(&rootNodeChildrenAllocator, node->size);
...
static inline void expandRootNode(RootNode *node) {
...
static inline void extendRootNodeActiveInterval(RootNode *node, int position) {
    ...
    for (; s <= position; s++) {
        ...
        // allocate zone node
        node->childZoneNodes[s] = (ZoneNode *) uallocate(&tAllocator, 1);
        initZoneNode(node->childZoneNodes[s],
                     node->model.invSlope * s + node->model.bias);
    }
}

static inline void slideRootNode(RootNode *node) {}

static inline void incrementZoneNodeUsedCount(ZoneNode *node) { node->usedCount++; }
static inline void incrementZoneNodeInvCount(ZoneNode *node) {
...
static inline ZoneNode *fetchZoneNodeFromRootNode(RootNode *node,
...
static inline MapSegment *fetchMapSegmentFromDataNode(ZoneNode *node,
                                               unsigned int logicalSliceAddr,
                                               int isAllocate,
                                               uint64_t *parent_addr, bool* last_in_mapseg) {
    int position = calcPosition(node->model, logicalSliceAddr);
    ...
    *parent_addr = (uint64_t) ((MapSegment_p) (&(node->childMapSegments[position])));
    if (pMapSegment == NULL && isAllocate) {
        pMapSegment = (MapSegment *) uallocate(&mapSegmentAllocator, 1);
...
static inline MapSegment *fetchMapSegmentFromFmTree(FunctionalMappingTree *fmTree,
                                             unsigned int logicalSliceAddr,
                                             int isAllocateChunk,
                                             uint64_t *parent_addr_ret, bool* last_in_mapseg) {
