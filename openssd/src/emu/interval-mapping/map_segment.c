...
struct unit_allocator rootNodeAllocator;
struct unit_allocator tAllocator;
struct unit_allocator mapSegmentAllocator;
struct unit_allocator rootNodeChildrenAllocator;
struct unit_allocator rangeDirAllocator;
struct unit_allocator rangeBufferAllocator;
struct unit_allocator mapSegmentBufferAllocator;
...
{
    init_unit_allocator(&rootNodeAllocator, sizeof(RootNode), &ssd_allocator);
    init_unit_allocator(&tAllocator, sizeof(ZoneNode), &ssd_allocator);
    init_unit_allocator(&mapSegmentAllocator, sizeof(MapSegment), &ssd_allocator);
    init_unit_allocator(&rootNodeChildrenAllocator, sizeof(ZoneNode *), &ssd_allocator);
    init_unit_allocator(&rangeDirAllocator, sizeof(uint16_t), &ssd_allocator);
    init_unit_allocator(&rangeBufferAllocator, sizeof(LOGICAL_SLICE_ENTRY), &ssd_allocator);
    init_unit_allocator(&mapSegmentBufferAllocator, sizeof(RangeBuffer), &ssd_allocator);
}

// MapSegment *mapseg_select_map_segment(unsigned int logicalSliceAddr,
//                                       int isAllocate, uint64_t *parent_addr_ret);
...
void mapseg_init_map_segment(MapSegment *pMapSegment, unsigned int startLsa) {
    ...
    pMapSegment->mappingSize = MAPSEG_MAP_SEGMENT_SIZE; // one map node manages 16MB (= 4K * 4KB) space 
    pMapSegment->unitRangeSize = MAPSEG_MAP_SEGMENT_SIZE;   // initial unit range size 
                                                            // equals to the full range size
    ...
    unsigned int dirSize =
        pMapSegment->mappingSize / pMapSegment->unitRangeSize;  // 1
    // init directory
    ...
    pMapSegment->rangeBuffer =
        (RangeBuffer *) uallocate(&mapSegmentBufferAllocator, MAPSEG_INITIAL_BUFFER_SIZE);
...
// search interval tree to get the map segment of the given logical slice address
MapSegment *mapseg_select_map_segment(unsigned int logicalSliceAddr,
                                      int isAllocate, uint64_t *parent_addr_ret,
									  bool* last_in_mapseg) {
...
// 		xil_printf("%s: access to compacted shival nyuan 2\n", __func__);
    ...
    if (virtualSliceAddr != VSA_NONE)
        mapseg_mark_valid(selectedMapSegment, indexInMapSegment, 1,
						  ...
						  false);
    else    // entering this branch only when called by `mapseg_remove`
        mapseg_mark_valid(selectedMapSegment, indexInMapSegment, 1,
...
// remove mapping of given `logicalSliceAddr`, set it as `VSA_NONE`
int mapseg_remove(unsigned int logicalSliceAddr, bool last_discard) {
    ...
    // last_in_mapseg: true if this mapseg is the last one in the zone node.
...
void mapseg_deallocate(MapSegment_p wchunk_p, unsigned int chunkStartAddr) {
    ...
    // not implemented ? incremental allocation is enough?
}
...
static bool no_more_write(MapSegment_p wchunk_p)
{
...
static bool is_compaction_candidate(MapSegment_p wchunk_p)
{
    return wchunk_p->numOfValidMaps <=
		wchunk_p->mappingSize * COMPACTION_CONDITION_RATIO;
}

static int TEST_BITMAP(unsigned int* bitmap, int i)
{
    int validBitIndex, validBitSelector;
...
static void GET_RANGE_SIZE(unsigned int* bitmap,  uint32_t n_maps,
...
static int GET_NEXT_VALID_BIT(unsigned int* bitmap, uint32_t sidx,
...
static int GET_NEXT_INVALID_BIT(unsigned int* bitmap, uint32_t sidx,
                                uint32_t eidx, bool print)
{
        int i;
//        if (print)
...
static uint32_t GET_REMAINING_INVALID_MAPPING(unsigned int* bitmap,
...
static uint32_t GET_MIN_HOLE_SIZE(unsigned int* bitmap,
                        uint32_t first_valid_idx, uint32_t last_valid_idx,
...
static void SET_BITMAP(unsigned int* bitmap, int i)
{
...
static int COPY_BITMAP_MASK(unsigned int* dst_bitmap, uint32_t dst_index,
...
static MapSegment_p CONSTRUCT_COMPACTED_MAP_NODE(MapSegment_p old_mnode,
								...
								uint32_t n_active_regions)
{
	...
	MapSegment_p mnode;
        ...
        mnode = (MapSegment_p) uallocate(&mapSegmentAllocator, 1);
...
//			first_valid_idx, last_valid_idx, range_directory_sz, bitmap_sz, mapping_table_sz);
	...
	/* initialize rangeBuffer */
    	mnode->rangeBuffer = (RangeBuffer *) uallocate(
            &mapSegmentBufferAllocator, MAPSEG_INITIAL_BUFFER_SIZE);
	    mnode->rangeBuffer->entries = (LOGICAL_SLICE_ENTRY *) uallocate(
        &rangeBufferAllocator, mapping_table_sz/sizeof(LOGICAL_SLICE_ENTRY));
...
//            			sidx, last_valid_idx, eidx);
                ...
                for (ii = cur_dir_idx; ii <= cur_dir_eidx; ii += 1){
                    // one active region shared by multiple region has one header only
...
static MapSegment_p try_compaction(MapSegment_p wchunk_p, bool print)
{
        nsleep(1000*1000); // 1ms
        return NULL;
...
//                	xil_printf("%s: 4\n", __func__);
        ...
        if (min_hole > MIN_REGION){ //  1 consecutive space in one region
//        	if (print)
//        	        	xil_printf("%s: 5\n", __func__);
                /* # of headers + # of mappings*/
                n_entries_fixed_region_mappings =
                        wchunk_p->numOfValidMaps + n_active_regions;
        } else {    // may exist >= 2 consecutive spaces in one region
//				if (print)
//							xil_printf("%s: 6\n", __func__);
...
//        		xil_printf("shival ------ 2\n");
//        }
    	if (last_discard){  // true when this is called in DSM handling path 
                            //  or current mapseg is last one in the zone node
...
//				xil_printf("%s: last discard idx: %d\n", __func__, indexInChunk);
	...
	//			    }
					...
					xil_printf("oldsz: %d newsz: %d compaction ratio(x100): %d\n", wchunk_p->sz,
							new_wchunk_p->sz,
							new_wchunk_p->sz *100 / wchunk_p->sz);
					uint64_t *parent_p = (uint64_t*)parent_addr;
	//				xil_printf("shival 1\n");
					*parent_p = (uint64_t)new_wchunk_p;
	//				xil_printf("shival 2\n");
					target = new_wchunk_p;
