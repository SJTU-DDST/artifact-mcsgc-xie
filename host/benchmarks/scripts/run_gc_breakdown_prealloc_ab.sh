...
if [ -z "${GC_BREAKDOWN_PREALLOC_AB_SNAPSHOT:-}" ]; then
    script_path=$(readlink -f -- "${BASH_SOURCE[0]}")
    script_body=$(<"${script_path}")
    export GC_BREAKDOWN_PREALLOC_AB_SNAPSHOT=1
    export GC_BREAKDOWN_PREALLOC_AB_SCRIPT_PATH="${script_path}"
    exec /bin/bash -c "${script_body}" "${script_path}" "$@"
fi

script_path=${GC_BREAKDOWN_PREALLOC_AB_SCRIPT_PATH}
...
comparator="${script_dir}/draw-xie/compare-gc-prealloc-results.py"
batch_id=$(date +"%Y%m%d_%H%M%S")
batch_dir="${script_dir}/outputs-gc-breakdown-prealloc-ab/${batch_id}"
...
Builds and runs these four full diagnostics on SSD1t:
  1. PRE allocation control, bigfile
  2. PRE allocation control, smallfile
  3. dirty-batched PRE allocation, bigfile
  4. dirty-batched PRE allocation, smallfile
...
# Run a short treatment workload before committing to the four full runs.
run_treatment_smoke() {
    local result_path_file="${batch_dir}/00-treatment-bigfile-smoke.result-path"
    local run_dir
    local fsck_log

    echo
    echo "Starting treatment bigfile smoke at $(date --iso-8601=seconds)"
    sudo env FIO_IO_SIZE_PER_THREAD_OVERRIDE=1G \
        GC_BREAKDOWN_RESULT_PATH_FILE="${result_path_file}" \
        "${runner}" mcsgc8t-prealloc-dirty-batch bigfile
    if [ ! -s "${result_path_file}" ]; then
        echo "ERROR: treatment smoke result path was not recorded." >&2
        return 1
    fi
    run_dir=$(<"${result_path_file}")
    if [ ! -s "${run_dir}/gc-breakdown-diagnostic-result.txt" ]; then
        echo "ERROR: treatment smoke diagnostic summary is missing." >&2
        return 1
    fi
...
prepare_configuration mcsgc8t-prealloc-dirty-batch \
    exp/diagnostic-mcsgc8t-prealloc-dirty-batch-20260818
run_treatment_smoke

prepare_configuration mcsgc8t-prealloc-control \
    exp/diagnostic-mcsgc8t-prealloc-control-20260818
run_one mcsgc8t-prealloc-control bigfile 1
run_one mcsgc8t-prealloc-control smallfile 2

prepare_configuration mcsgc8t-prealloc-dirty-batch \
    exp/diagnostic-mcsgc8t-prealloc-dirty-batch-20260818
run_one mcsgc8t-prealloc-dirty-batch bigfile 3
run_one mcsgc8t-prealloc-dirty-batch smallfile 4

comparison="${batch_dir}/comparison.txt"
python3 "${comparator}" \
    --control-big "$(<"${batch_dir}/01-mcsgc8t-prealloc-control-bigfile.result-path")" \
    --control-small "$(<"${batch_dir}/02-mcsgc8t-prealloc-control-smallfile.result-path")" \
    --treatment-big "$(<"${batch_dir}/03-mcsgc8t-prealloc-dirty-batch-bigfile.result-path")" \
    --treatment-small "$(<"${batch_dir}/04-mcsgc8t-prealloc-dirty-batch-smallfile.result-path")" \
    --output "${comparison}"
...
echo
echo "PRE allocation control/treatment diagnostics completed successfully."
