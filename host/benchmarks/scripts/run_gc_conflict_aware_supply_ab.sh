...
if [ -z "${GC_CONFLICT_SUPPLY_AB_SNAPSHOT:-}" ]; then
    script_path=$(readlink -f -- "${BASH_SOURCE[0]}")
    script_body=$(<"${script_path}")
    export GC_CONFLICT_SUPPLY_AB_SNAPSHOT=1
    export GC_CONFLICT_SUPPLY_AB_SCRIPT_PATH="${script_path}"
    exec /bin/bash -c "${script_body}" "${script_path}" "$@"
fi

script_path=${GC_CONFLICT_SUPPLY_AB_SCRIPT_PATH}
...
comparator="${script_dir}/draw-xie/compare-conflict-aware-supply-results.py"
batch_id=$(date +"%Y%m%d_%H%M%S")
batch_dir="${script_dir}/outputs-gc-conflict-aware-supply-ab/${batch_id}"
manifest="${batch_dir}/results.txt"
sudo_keepalive_pid=""

declare -A branches=(
    [mcsgc8t-conflict-aware-supply]=exp/diagnostic-mcsgc8t-conflict-aware-supply-20260819
    [mcsgc8t-rolling-supply]=exp/diagnostic-mcsgc8t-rolling-supply-20260819
)
...
Builds the conflict-aware and rolling Host modules, runs two rolling smoke
tests, then runs both full workloads on both modules. Every benchmark resets
and reformats /dev/nvme0n1. Run this outer script as the login user.
EOF
}
...
build_configuration mcsgc8t-conflict-aware-supply
build_configuration mcsgc8t-rolling-supply

load_configuration mcsgc8t-rolling-supply
run_one mcsgc8t-rolling-supply bigfile 0 1
run_one mcsgc8t-rolling-supply smallfile 1 1

load_configuration mcsgc8t-conflict-aware-supply
run_one mcsgc8t-conflict-aware-supply bigfile 2
run_one mcsgc8t-conflict-aware-supply smallfile 3

load_configuration mcsgc8t-rolling-supply
run_one mcsgc8t-rolling-supply bigfile 4
run_one mcsgc8t-rolling-supply smallfile 5

comparison="${batch_dir}/comparison.txt"
python3 "${comparator}" \
    --control-big "$(<"${batch_dir}/02-mcsgc8t-conflict-aware-supply-bigfile.result-path")" \
    --control-small "$(<"${batch_dir}/03-mcsgc8t-conflict-aware-supply-smallfile.result-path")" \
    --rolling-big "$(<"${batch_dir}/04-mcsgc8t-rolling-supply-bigfile.result-path")" \
    --rolling-small "$(<"${batch_dir}/05-mcsgc8t-rolling-supply-smallfile.result-path")" \
    --output "${comparison}"

printf '\ncompleted_at=%s\nstatus=success\ncomparison=%s\n' \
    "$(date --iso-8601=seconds)" "${comparison}" >> "${manifest}"
echo "Conflict-aware supply A/B completed: ${batch_dir}"
