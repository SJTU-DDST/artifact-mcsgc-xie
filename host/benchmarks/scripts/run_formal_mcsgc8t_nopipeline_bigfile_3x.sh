...
if [ ! -r "${module_path}" ]; then
    die "formal Host module is unavailable; run './prepare_formal_host_module.sh mcsgc8t-nopipeline' first"
fi

echo "============================================================"
echo "Formal optimized mCSGC8t no-pipeline single-big-file benchmark"
...
for ((round = 1; round <= repeats; round++)); do
    ...
    if ! "${runner}" "${configuration}" "${workload}"; then
        ...
        echo "The remaining mCSGC8t runs were not started." >&2
        exit 1
    fi
...
echo
echo "All three formal optimized mCSGC8t single-big-file runs completed successfully."
