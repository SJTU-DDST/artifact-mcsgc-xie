...
if [ ! -r "${module_path}" ]; then
    die "formal Host module is unavailable; run './prepare_formal_host_module.sh original-csgc' first"
fi

echo "============================================================"
echo "Formal original CSGC single-big-file benchmark"
...
for ((round = 1; round <= repeats; round++)); do
    ...
    if ! "${runner}" "${configuration}" "${workload}"; then
        ...
        echo "The remaining original CSGC runs were not started." >&2
        exit 1
    fi
...
echo
echo "All three formal original CSGC single-big-file runs completed successfully."
