...
if [ -z "${GC_GAP_UNSAFE_AB_SNAPSHOT:-}" ]; then
    script_path=$(readlink -f -- "${BASH_SOURCE[0]}")
    script_body=$(<"${script_path}")
    export GC_GAP_UNSAFE_AB_SNAPSHOT=1
    export GC_GAP_UNSAFE_AB_SCRIPT_PATH="${script_path}"
    exec /bin/bash -c "${script_body}" "${script_path}" "$@"
fi

script_path=${GC_GAP_UNSAFE_AB_SCRIPT_PATH}
...
comparator="${script_dir}/draw-xie/compare-gc-gap-unsafe-results.py"
batch_id=$(date +"%Y%m%d_%H%M%S")
batch_dir="${script_dir}/outputs-gc-gap-unsafe-ab/${batch_id}"
manifest="${batch_dir}/results.txt"
sudo_keepalive_pid=""

declare -A branches=(
    [mcsgc8t-gc-gap-control]=exp/diagnostic-mcsgc8t-gc-gap-control-20260818
    [mcsgc8t-unsafe-prefree-reclaim]=exp/diagnostic-mcsgc8t-unsafe-prefree-reclaim-20260818
    [mcsgc8t-unsafe-prefree-refill]=exp/diagnostic-mcsgc8t-unsafe-prefree-refill-20260818
)
declare -A module_paths=()

# Print the destructive one-command workflow without touching the SSD.
usage() {
    cat <<EOF
Usage: ./$(basename -- "${script_path}")

Builds three Host configurations, runs two shortened treatment smoke tests,
then runs six full SSD1t diagnostics:
  control/reclaim/refill x bigfile/smallfile
...
# Read OpenSSD provenance without modifying the remote server.
...
# Replace the unloaded F2FS module with one already built configuration.
load_configuration() {
    local configuration=$1
...
# Run one workload and record its exact output paths.
run_one() {
    local configuration=$1
    local workload=$2
    local sequence=$3
    ...
    local run_dir
    local -a environment=(GC_BREAKDOWN_RESULT_PATH_FILE=)
    ...
    if [ "${smoke}" -eq 1 ]; then
        environment+=("GC_BREAKDOWN_SMOKE=1")
    fi
...
if [ "${openssd_commit}" != "${expected_openssd_commit}" ]; then
    echo "ERROR: expected OpenSSD commit ${expected_openssd_commit}, got ${openssd_commit}." >&2
    exit 1
fi
...
build_configuration mcsgc8t-gc-gap-control
build_configuration mcsgc8t-unsafe-prefree-reclaim
build_configuration mcsgc8t-unsafe-prefree-refill

load_configuration mcsgc8t-unsafe-prefree-reclaim
run_one mcsgc8t-unsafe-prefree-reclaim bigfile 0 1
load_configuration mcsgc8t-unsafe-prefree-refill
run_one mcsgc8t-unsafe-prefree-refill bigfile 1 1

load_configuration mcsgc8t-gc-gap-control
run_one mcsgc8t-gc-gap-control bigfile 2
run_one mcsgc8t-gc-gap-control smallfile 3

load_configuration mcsgc8t-unsafe-prefree-reclaim
run_one mcsgc8t-unsafe-prefree-reclaim bigfile 4
run_one mcsgc8t-unsafe-prefree-reclaim smallfile 5

load_configuration mcsgc8t-unsafe-prefree-refill
run_one mcsgc8t-unsafe-prefree-refill bigfile 6
run_one mcsgc8t-unsafe-prefree-refill smallfile 7

comparison="${batch_dir}/comparison.txt"
python3 "${comparator}" \
    --control-big "$(<"${batch_dir}/02-mcsgc8t-gc-gap-control-bigfile.result-path")" \
    --control-small "$(<"${batch_dir}/03-mcsgc8t-gc-gap-control-smallfile.result-path")" \
    --reclaim-big "$(<"${batch_dir}/04-mcsgc8t-unsafe-prefree-reclaim-bigfile.result-path")" \
    --reclaim-small "$(<"${batch_dir}/05-mcsgc8t-unsafe-prefree-reclaim-smallfile.result-path")" \
    --refill-big "$(<"${batch_dir}/06-mcsgc8t-unsafe-prefree-refill-bigfile.result-path")" \
    --refill-small "$(<"${batch_dir}/07-mcsgc8t-unsafe-prefree-refill-smallfile.result-path")" \
    --output "${comparison}"
...
echo
echo "GC gap unsafe A/B completed successfully."
