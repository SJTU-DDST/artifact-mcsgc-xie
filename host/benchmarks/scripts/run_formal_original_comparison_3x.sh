...
if [ ! -r "${module_path}" ]; then
    ...
    echo "Action: run './prepare_formal_host_module.sh original-csgc' first." >&2
    exit 1
fi

echo "============================================================"
echo "Formal original ORI/CSGC comparison"
echo "Host branch: ${actual_branch}"
echo "Host commit: $(git -C "${host_tree}" rev-parse HEAD)"
echo "Repeats per configuration: ${repeats}"
echo "Execution order per round: original-ori, original-csgc"
echo "============================================================"

for ((round = 1; round <= repeats; round++)); do
    for configuration in original-ori original-csgc; do
        echo
        echo "============================================================"
        ...
        if "${runner}" "${configuration}"; then
            ...
            echo "The remaining runs were not started." >&2
            exit "${status}"
        fi
    done
done
...
echo
echo "All six formal original ORI/CSGC runs completed successfully."
