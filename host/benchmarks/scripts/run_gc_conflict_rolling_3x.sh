...
if [ -z "${GC_CONFLICT_ROLLING_3X_SNAPSHOT:-}" ]; then
    script_path=$(readlink -f -- "${BASH_SOURCE[0]}")
    script_body=$(<"${script_path}")
    export GC_CONFLICT_ROLLING_3X_SNAPSHOT=1
    export GC_CONFLICT_ROLLING_3X_SCRIPT_PATH="${script_path}"
    exec /bin/bash -c "${script_body}" "${script_path}" "$@"
fi

script_path=${GC_CONFLICT_ROLLING_3X_SCRIPT_PATH}
...
batch_dir="${script_dir}/outputs-gc-conflict-rolling-3x/${batch_id}"
...
declare -A branches=(
    [conflict-aware]=exp/diagnostic-mcsgc8t-conflict-aware-lifecycle-fast-20260825
    [rolling-final]=exp/diagnostic-mcsgc8t-rolling-lifecycle-fast-20260825
)
declare -A commits=(
...
declare -A base_commits=(
    [conflict-aware]=62f0a68a891bf39e14398e5d08a083ee79fe73fe
    [rolling-final]=72770bf7202618f1c23575e830d1851aebd47cf7
)
declare -A runner_configs=(
    [conflict-aware]=mcsgc8t-conflict-aware-lifecycle-fast
    [rolling-final]=mcsgc8t-rolling-lifecycle-fast
)
declare -A preferred_worktrees=(
    [conflict-aware]=/home/xin/work-xie/mcsgc-real/linux-cs-conflict-aware-lifecycle-fast-20260825
    [rolling-final]=/home/xin/work-xie/mcsgc-real/linux-cs-rolling-lifecycle-fast-20260825
)
...
Builds these pinned low-overhead lifecycle-fixed Host revisions once:
  conflict-aware  2646852dafefd103da96cb34c7c8f958b3643136
  rolling-final   441f1b1f6449e4f9ea2e2b35c401669a9e29b4d3
...
Use tmux for resilience during the approximately 2 hour 15 minute batch:
  tmux new-session -s csgc-3x './$(basename -- "${script_path}")'
EOF
}
...
run_one() {
    ...
    [ -s "${run_dir}/gc-breakdown-diagnostic-result.txt" ] \
        || die "diagnostic summary is missing for ${label}"
...
with open(markdown_path, "w", encoding="utf-8") as output:
    output.write("# Conflict-aware and Rolling-final 3x Results\n\n")
...
[ -x "${runner}" ] || die "diagnostic runner is unavailable: ${runner}"
...
exec 9>/tmp/run_gc_conflict_rolling_3x.lock
flock -n 9 || die "another conflict/rolling 3x batch is already running"
...
echo
echo "All 12 conflict-aware and rolling-final benchmarks completed successfully."
