...
case "${REPETITIONS}" in
    ''|*[!0-9]*|0)
        echo "ERROR: EUROPAR_NOWAIT_REPETITIONS must be a positive integer" >&2
        exit 2
        ;;
esac
...
Set EUROPAR_NOWAIT_REPETITIONS to override the default three repetitions.
EOF
}
...
stop_sudo_keepalive
trap - EXIT
echo "EUROPAR_MCSGC_NOWAIT_MATRIX_COMPLETE status=success cases=${successful} validated=${validated} completed_at=${COMPLETED_AT}"
