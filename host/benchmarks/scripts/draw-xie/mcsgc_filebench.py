...
fig_path = "./figs/mcsgc-compare-csgc.png"   # ← 改为 PNG
fig_size = (10, 2.3)
...
# mCSGC-t2（原 mCSGC）
mcs_t2_label    = 'mCSGC-t2'         # ← 重命名
mcs_t2_color    = '#65C295'
marker_mcs_t2   = 's'

# mCSGC-t1（新增）
mcs_t1_label    = 'mCSGC-t1'         # ← 新增
mcs_t1_color    = '#6595C2'          # ← 选一个与其它不同的颜色
marker_mcs_t1   = '^'                # ← 三角形标记
...
# CSGC 日志路径
# "/home/xin/work-xie/csgc/xin_scripts/outputs-cs/20250427_235503/"
...
# mCSGC-t2 日志路径（原 mCSGC）
mcs_t2_log = [
    "/home/xin/work-xie/csgc/xin_scripts/outputs-cs-t2/20250623_115622/filebench_fileserver_4t_60G_1M_54k_s8/filebench.log"
]
# mCSGC-t1 日志路径（新增）
#     "/home/xin/work-xie/csgc/xin_scripts/outputs-cs-t1/20250428_102046/filebench_fileserver_4t_60G_1M_54k_s8/filebench.log",
mcs_t1_log = [
    "/home/xin/work-xie/csgc/xin_scripts/outputs-cs-t2/20250604_145649/filebench_fileserver_4t_60G_1M_54k_s8/filebench.log"
]
...
start_t2    = mcs_t2_time[0]        # ← mCSGC-t2
...
start_t1    = mcs_t1_time[0]        # ← mCSGC-t1
...
# mCSGC-t2 折线
ax.plot(
    mcs_t2_time, mcs_t2_opps_k,
    label=mcs_t2_label,
    color=mcs_t2_color,
    marker=marker_mcs_t2,
    markersize=markersize
)
# mCSGC-t1 折线
ax.plot(
    mcs_t1_time, mcs_t1_opps_k,
    label=mcs_t1_label,
    color=mcs_t1_color,
    marker=marker_mcs_t1,
    markersize=markersize
)
...
print("csgc_total_kop=",end='')
print(csgc_total_kop)
print("mcs_t2_total_kop=",end='')
print(mcs_t2_total_kop)
print("mcs_t1_total_kop=",end='')
print(mcs_t1_total_kop)
# 在各条折线末尾标注总操作量
ax.text(
    csgc_time[-1] + 10, csgc_opps_k[-1],
    f'total={csgc_total_kop:.0f}K',
    color=csgc_color, ha='left', va='bottom',
    fontsize=fontsize2
)
ax.text(
    mcs_t2_time[-1] + 10, mcs_t2_opps_k[-1],   # ← t2
    f'total={mcs_t2_total_kop:.0f}K',
    color=mcs_t2_color, ha='left', va='bottom',
    fontsize=fontsize2
)
ax.text(
    mcs_t1_time[-1] + 10, mcs_t1_opps_k[-1],   # ← t1
    f'total={mcs_t1_total_kop:.0f}K',
    color=mcs_t1_color, ha='left', va='bottom',
    fontsize=fontsize2
)
...
plt.tight_layout()
# plt.savefig(fig_path, bbox_inches="tight", format="png")  # ← 输出 PNG
