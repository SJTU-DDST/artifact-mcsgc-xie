...
configuration="mcsgc8t-nopipeline"
repeats=3
...
if [[ ! -r "${module_path}" ]]; then
    die "formal mCSGC8t no-pipeline Host module has not been built. Run './prepare_formal_host_module.sh mcsgc8t-nopipeline' first"
fi

banner "Formal optimized mCSGC8t no-pipeline benchmark"
...
for ((run = 1; run <= repeats; run++)); do
    banner "mCSGC8t no-pipeline run ${run}/${repeats}"

    if ! "${runner}" "${configuration}"; then
        echo "ERROR: mCSGC8t no-pipeline run ${run}/${repeats} failed." >&2
        echo "The remaining mCSGC8t no-pipeline runs were not started." >&2
        exit 1
    fi
done

banner "All three formal optimized mCSGC8t no-pipeline runs completed successfully"
