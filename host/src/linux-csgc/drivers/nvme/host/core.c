...
#define CS_SEQ_ID_SHIFT 3 // dword12: 0_0_0000_0_0_[00000]_0_0_0
#define CS_SEQ_ID_MASK 0b11111000
#define CS_SEQ_ID_MAX ((1 << CS_SEQ_ID_BITS) - 1)
		...
		enum nvme_opcode op)
{
	...
	if (req->cmd_flags & (REQ_FAILFAST_DEV | REQ_RAHEAD))
		control |= NVME_RW_LR;
	if (req->cmd_flags & REQ_CSGC){
		unsigned int *cs_private = (unsigned int *)(req->bio->bi_private);
		unsigned int cs_seq_id = *((unsigned char *) cs_private);
		unsigned int cs_is_head = *(((unsigned char *) cs_private) + 1);
		control |= NVME_RW_CSGC;
		control |= (cs_seq_id << CS_SEQ_ID_SHIFT) & CS_SEQ_ID_MASK;
		if(cs_is_head)
			control |= NVME_RW_CS_IS_HEAD;
	}
	if (req->cmd_flags & REQ_GCIO)
		control |= NVME_RW_GCIO_HOST;
