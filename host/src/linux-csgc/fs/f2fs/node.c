...
// if target node page is in page cache and uptodate, 
//		return it the same as `f2fs_get_node_page` (locked).
// if not cached or not uptodate, return %NULL.
struct page *f2fs_get_node_page_uptodate(struct f2fs_sb_info *sbi, pgoff_t nid)
{
	struct page *page;
	page = find_get_page(NODE_MAPPING(sbi), nid);
	if(page && PageUptodate(page)){
		f2fs_debug_csgc("node page <nid=%lu> is cached and uptodate, needs update", nid);
		f2fs_put_page(page, 0);
		return __get_node_page(sbi, nid, NULL, 0);
	}
	f2fs_debug_csgc("node page <nid=%lu> not cached or not uptodate, no need to update", nid);
	return NULL;
}
...
#ifdef CONFIG_F2FS_TRACK_NODE_WRITEBACK
	f2fs_debug_csgc_pid("Write back node page(nid=%lu) to blkaddr=%u", page->index, fio.new_blkaddr);
#endif
			    ...
			    struct writeback_control *wbc)
{
	...
	blk_start_plug(&plug);
	// write back dirty nodes in f2fs NODE_MAPPING
...
{
	struct f2fs_node *node = (struct f2fs_node *) page_address(node_page);
	int start, end;
	unsigned int max_addrs = IS_INODE(node_page) ? addrs_per_inode(inode) : addrs_per_block(inode);
	if(nr_dump < 0 || nr_dump > max_addrs)
        nr_dump = max_addrs;
    ...
    printk(KERN_INFO "<NODE DUMP> NID:%u, INODE_NR:%u, FLAG:%#x, CP_VER:%#llx, NXT_BLK:%u \n", 
            node->footer.nid, node->footer.ino, node->footer.flag, 
            node->footer.cp_ver, node->footer.next_blkaddr);
    for(int i = start; i < end; i++){
        if(hex){
            printk(KERN_INFO "<NODE DUMP> Entry %d: nid/block_addr=0x%04x",
                 i, data_blkaddr(inode, node_page,i));
        }else{
            printk(KERN_INFO "<NODE DUMP> Entry %d: nid/block_addr=%u",
                 i, data_blkaddr(inode, node_page,i));
        }
    }
}