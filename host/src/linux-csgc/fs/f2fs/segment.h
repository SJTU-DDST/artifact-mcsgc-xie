...
#define SHOW_BITMAP_DEFAULT true
#else
#define SHOW_BITMAP_DEFAULT false
#endif

#pragma GCC diagnostic ignored "-Wunused-function"
static void __dump_sit_entry(struct seg_entry *sentry, int show_bitmap)
{
    unsigned char *valid_map;
	f2fs_debug_csgc( "<SIT DUMP>\t alloc type=%u, ckpt_valid_blocks=%u valid_blocks=%u, mtime=%llu\n",
				sentry->type,
				sentry->ckpt_valid_blocks,
                sentry->valid_blocks,
                sentry->mtime);
	if(show_bitmap < 0)
		show_bitmap = SHOW_BITMAP_DEFAULT;
	if(!show_bitmap)
		return;
	valid_map = sentry->cur_valid_map;
	f2fs_debug_csgc( "<SIT DUMP> current bitmap\n");
    ...
    {   
        f2fs_debug_csgc( "<SIT DUMP>     %02hhx %02hhx %02hhx %02hhx "
                    ...
                    "%02hhx %02hhx %02hhx %02hhx "
                    "%02hhx %02hhx %02hhx %02hhx\n",
                    valid_map[j], valid_map[j+1], valid_map[j+2], valid_map[j+3],
                    valid_map[j+4], valid_map[j+5], valid_map[j+6], valid_map[j+7],
                    valid_map[j+8], valid_map[j+9], valid_map[j+10], valid_map[j+11],
                    valid_map[j+12], valid_map[j+13], valid_map[j+14], valid_map[j+15]
                    );
    }
	valid_map = sentry->ckpt_valid_map;
	f2fs_debug_csgc( "<SIT DUMP> last ckpt bitmap\n");
    for(int j = 0; j < SIT_VBLOCK_MAP_SIZE; j+=16)
    {   
        f2fs_debug_csgc( "<SIT DUMP>     %02hhx %02hhx %02hhx %02hhx "
                    "%02hhx %02hhx %02hhx %02hhx "
...
#else
#pragma GCC diagnostic ignored "-Wunused-function"
static inline void __dump_sit_entry(struct seg_entry *sentry, int show_bitmap) {}
#endif
