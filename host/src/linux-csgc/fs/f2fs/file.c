...
// set addrs in the range [dn->ofs_in_node, dn->ofs_in_node + count) to NULL_ADDR, 
// and invalidate them in SIT bitmap
...
// Buffered io does not need to lock gc_rwsem to prevent the write to 
// the whole inode. The only contention is between the [generic write of
// the data page] and the [migration of the data page]. So locking the page
// in these 2 processes is enough. 
					...
					struct iov_iter *from)
{
	...
	// in `generic_perform_write`, it will call `f2fs_write_begin`(by `a_ops->write_begin`) 
	// to get the page(locked) to be written
				   ...
				   bool *may_need_sync)
{
...
#ifdef CONFIG_F2FS_TRACK_DIO
	printk(KERN_INFO "f2fs_dio_write_iter: ino=%lu acquired i_gc_rwsem[READ] pos=%lld, count=%ld", inode->i_ino, pos, count);
#endif
	/*
	 * We have to use __iomap_dio_rw() and iomap_dio_complete() instead of
	 * the higher-level function iomap_dio_rw() in order to ensure that the
	...
	f2fs_up_read(&fi->i_gc_rwsem[WRITE]);
#ifdef CONFIG_F2FS_TRACK_DIO
	printk(KERN_INFO "f2fs_dio_write_iter: ino=%lu released i_gc_rwsem[READ]", inode->i_ino);
#endif
