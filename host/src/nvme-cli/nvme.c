		     ...
		     int argc, char **argv)
{
	...
	const char *storage_tag = "storage tag, CDW2 and CDW3 (00:47) bits "\
		"(for end to end PI)";
	const char *cs = "set computational storage flag";
	const char *getlog = "request ssd output log";
	const char *itvl_log = "set gc invalid block interval";
	const char *n_segs = "set number of segments to do gc";
	...
	OPT_ARGS(opts) = {
		...
		OPT_FLAG("cs",                'P', &cfg.cs,                cs),
		OPT_FLAG("getlog",            'L', &cfg.get_ssd_log,       getlog),
		OPT_UINT("itvl-log",          'i', &cfg.itvl_log,          itvl_log),
		OPT_UINT("nsegs",             'N', &cfg.n_segs,            n_segs),
		OPT_END()
	};
	// for gc simulation
	// sudo ./nvme write /dev/nvme0n1 -s 0 -c 1 -z 4096 -d in.bin -t -P -i 0 -N 1

	// for true gc
	// sudo ./nvme write /dev/nvme0n1 -s 0 -c 1 -z 4096 -d in.bin -t -P -N 4
	...
	// if (cfg.cs)
	// 	if (opcode != nvme_cmd_write ||	cfg.start_block != 0) {
	// 		errno = EINVAL;
	// 		err = -1;
	// 		goto close_fd;
	// 	}
	...
	if (cfg.cs){ // if cs request, set control bit(not support openSSD)
	// Format for GC test
	// 0000_0000_0000_0|00|0
	// bit 0   : CS bit
	// bit 1~2 : itvl_log
	// bit 3~15: n_segs
		// control = 0;
		// control |= 1; // CS bit
		// control |= (cfg.itvl_log << 1) & (6) ; // mask = 6 = 0000_0000_0000_0110
		// control |= cfg.n_segs << 3;
	// Format for real GC
	// 0000_0000_0000_000|0
	// bit 0   : CS bit
	// bit 1~15: segno of the segment to be GCed
		control = 0;
		control |= 1;
		control |= cfg.n_segs << 1;
	}

	if (cfg.get_ssd_log){
	// for OpenSSD only, request to get the ssd log containing stats
		control = 0;
		control |= 1 << 2;
	}
	...
	err = nvme_io(fd, opcode, cfg.namespace_id, cfg.start_block, cfg.block_count,
			 ...
			 cfg.storage_tag, buffer, buffer_size, mbuffer, mbuffer_size, &result);
	...
	if (err < 0)
		...
		if (!(opcode & 1) && write(dfd, (void *)buffer, cfg.data_size) < 0) {
			...
			if (cfg.cs)
				fprintf(stderr, "CS result: %u (0x%08x)\n", result, result);
		}
	}
...
// example usage
// To get openssd log: 
//		sudo ./nvme read /dev/nvme1n1 -s 123 -c 1 -z 4096 -t -L
...
static int signal_raid_status(int argc, char **argv, const char *desc, int is_ready)
{
	int fd, err;
	...
	cmd.opcode = nvme_admin_raid_status;
	cmd.cdw10 = is_ready;
...
static int raid_ready_cmd(int argc, char **argv, struct command *command, struct plugin *plugin)
{
	const char *desc = "Signal the device that RAID is ready";
	return signal_raid_status(argc, argv, desc, 1);
}

static int raid_stop_cmd(int argc, char **argv, struct command *command, struct plugin *plugin)
{
	const char *desc = "Signal the device that RAID has been stopped";
	return signal_raid_status(argc, argv, desc, 0);
}

static int signal_fs_status(int argc, char **argv, const char *desc, int is_ready)
{
	int fd, err;
	struct nvme_admin_cmd cmd;

	struct config {
		int fs_type; // 0 for ext4, 1 for f2fs
	};
	...
	OPT_ARGS(opts) = {
		OPT_INT("fs_type", 'f', &cfg.fs_type, "file system type, 0 for ext4, 1 for f2fs"),
		OPT_END()
	};
	...
	memset(&cmd, 0, sizeof(struct nvme_admin_cmd));
	cmd.opcode = nvme_admin_fs_status;
	cmd.cdw10 = is_ready;
	cmd.cdw11 = cfg.fs_type;
...
static int signal_io_test(int argc, char **argv, const char *desc)
{
	int fd, err;
	struct nvme_admin_cmd cmd;
	...
	OPT_ARGS(opts) = {
		OPT_UINT("rw", 't', &cfg.rw, "test type, 0 for read, 1 for write, 2 for d2d"),
		OPT_UINT("addr", 'a', &cfg.blkaddr, "block address to read/write"),
		OPT_UINT("size", 's', &cfg.size, "byte size to read/write"),
		OPT_UINT("dst", 'd', &cfg.blkaddr2, "dest block address to read/write"),
		OPT_END()
	};
	...
	cmd.cdw10 = cfg.rw;
	cmd.cdw11 = cfg.blkaddr;
	cmd.cdw12 = cfg.size;
	cmd.cdw13 = cfg.blkaddr2;
...
static int signal_ssd_admin(int argc, char **argv, const char *desc)
{
	...
	struct ssd_config{
		int l2p_mapping_enabled;
		int nand_latency_emu_enabled;
    	int dsm_enabled;
	};
...
static int fs_ready_cmd(int argc, char **argv, struct command *command, struct plugin *plugin)
{
	const char *desc = "Signal the device that file system is ready";
	return signal_fs_status(argc, argv, desc, 1);
}

static int io_test_cmd(int argc, char **argv, struct command *command, struct plugin *plugin)
{
...
static int fs_stop_cmd(int argc, char **argv, struct command *command, struct plugin *plugin)
{
	const char *desc = "Signal the device that file system has been stopped";
	return signal_fs_status(argc, argv, desc, 0);
}

static inline uint64_t time_diff_ms(struct timeval end, struct timeval begin)
{
    return (end.tv_sec - begin.tv_sec)*1000 + (end.tv_usec - begin.tv_usec)/1000;
}

static int get_inode_cmd(int argc, char **argv, struct command *command, struct plugin *plugin)
{
	const char *desc = "Get specified inode info";
	int fd, err;
	struct nvme_admin_cmd cmd;
	...
	OPT_ARGS(opts) = {
		OPT_INT("inode_nr", 'n', &cfg.inode_nr, "inode number"),
		OPT_END()
	};
	...
	memset(&cmd, 0, sizeof(struct nvme_admin_cmd));
	cmd.opcode = nvme_admin_get_inode;
	cmd.cdw10 = cfg.inode_nr;

	printf("Resueqting inode %d\n", cmd.cdw10);
...
static int dump_blk_cmd(int argc, char **argv, struct command *command, struct plugin *plugin)
{
	const char *desc = "Dump specified block";
	int fd, err;
	struct nvme_admin_cmd cmd;
	...
	struct config {
		int blk_type; // reference enum above
		uint32_t addr;
		uint32_t dump_nr; //number of entries to dump
	};
	...
	OPT_ARGS(opts) = {
		OPT_INT("type", 't', &cfg.blk_type, "block type"),
		OPT_UINT("addr", 'a', &cfg.addr, "inode: inode_number ; nat_blk: nat_blk offset ; node: node_id ; raw: block_addr"),
		OPT_UINT("num", 'n', &cfg.dump_nr, "number of entries to dump"),
		OPT_END()
	};
	...
	if (cfg.blk_type < 0 || cfg.blk_type >= NR_DUMP_TYPE) {
		perror("not supported dump mode");
		errno = EINVAL;
		err = -1;
		goto close_fd;
	}
	...
	memset(&cmd, 0, sizeof(struct nvme_admin_cmd));
	cmd.opcode = nvme_admin_dump_blk;
	cmd.cdw10 = cfg.blk_type;
	cmd.cdw11 = cfg.addr;
	cmd.cdw12 = cfg.dump_nr;

	printf("Dumping block(%d) %u\n", cmd.cdw10, cmd.cdw11);
...
static int gc_test_command(int argc, char **argv, struct command *command, struct plugin *plugin)
{
	const char *desc = "Test gc performance ";
	int fd, err;
	struct nvme_admin_cmd cmd;
	...
	OPT_ARGS(opts) = {
		OPT_UINT("nsegs", 'n', &cfg.n_segments, "# of segments to perform gc"),
		OPT_UINT("itvl", 'i', &cfg.invalid_interval, "every `itvl` blocks exits one invalid blocks"),
		OPT_END()
	};
	...
	if (cfg.n_segments > 7601) {
		perror("At most 7601 segments(14.xGiB)\n");
		errno = EINVAL;
		err = -1;
		goto close_fd;
	}
	...
	memset(&cmd, 0, sizeof(struct nvme_admin_cmd));
	cmd.opcode = nvme_admin_gc_test;
	cmd.cdw10 = cfg.n_segments;
	cmd.cdw11 = cfg.invalid_interval;

	printf("Performing GC test: %u segments, invalid_interval=%u\n", cmd.cdw10, cmd.cdw11);
...
static int fill_storage_command(int argc, char **argv, struct command *command, struct plugin *plugin)
{
	const char *desc = "Fill user segments with segno";
	int fd, err;
	struct nvme_admin_cmd cmd;
	struct timeval t1, t2;
	...
	memset(&cmd, 0, sizeof(struct nvme_admin_cmd));
	cmd.opcode = nvme_admin_fill_storage;

	printf("Fill user segments with segno to avoid reading zeros\n");
