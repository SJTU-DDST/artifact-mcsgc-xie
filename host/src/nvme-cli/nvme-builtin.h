...
COMMAND_LIST(
	...
	ENTRY("read", "Submit a read command, return results", read_cmd) // read 
	ENTRY("write", "Submit a write command, return results", write_cmd) // write
	// ENTRY("write-gc", "Submit a cs-gc request, wrapped in write command, return results", write_cmd) // write with gc
	...
	ENTRY("raid-ready", "Signal the device that RAID is ready", raid_ready_cmd)
	...
	ENTRY("dump-blk", "Print specified block info", dump_blk_cmd)
	ENTRY("gc-test", "Test gc performance", gc_test_command)
	ENTRY("fill-storage", "Fill user segments with segno", fill_storage_command)
);
